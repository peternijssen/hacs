...
async def async_download_file(hass, url):
    ...
    if "tags/" in url:
        _LOGGER.debug("tags/ are in '%s', this is beeing removed, but this IS a bug.", url)
    ...
    with async_timeout.timeout(5, loop=hass.loop):
        ...
        if request.status == 200:
            ...
            _LOGGER.error("Got status code %s when trying to download %s", request.status, url)
    return result
...
async def download_integration(hass, integration):
    ...
    try:
        ...
        _LOGGER.error("This sucks! There was an issue downloading the integration - %s", error)
...
async def download_plugin(hass, plugin):
    ...
    try:
        if os.path.exists(plugin_dir):
            _LOGGER.debug("%s exist, deleting current content before download.", plugin_dir)
    ...
    try:
        ...
        if remotedir is None:
            try:
                ...
                _LOGGER.error("This sucks! We could not locate ANY files to download - %s", error)
                return
        ...
        _LOGGER.error("This sucks! There was an issue downloading the plugin - %s", error)
...
async def download_hacs(hass):
    ...
    try:
        ...
        try:
            ...
            _LOGGER.error("This sucks! There was an issue downloading 'frontend' - %s", error)

        # Download the content of "frontend/views"
        try:
            remote_dir = repo.get_dir_contents("custom_components/hacs/frontend/views", ref)
            ...
            _LOGGER.error("This sucks! There was an issue downloading 'frontend/views' - %s", error)

        # Download the content of "frontend/elements"
        try:
            remote_dir = repo.get_dir_contents("custom_components/hacs/frontend/elements", ref)
            ...
            _LOGGER.error("This sucks! There was an issue downloading 'frontend/elements' - %s", error)
        ...
        try:
            ...
            _LOGGER.error("This sucks! There was an issue downloading 'handler' - %s", error)
